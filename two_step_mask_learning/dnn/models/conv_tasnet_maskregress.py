"""!
@brief TasNet Wrapper for Mask estimation.
...
class CTN(nn.Module):
    ...
    def infer_source_signals(self, mixture_wav, sources_masks=None):
        ...
        if sources_masks is None:
            sources_masks = self.forward(mixture_wav)
    ...
    def load_best_model(cls, models_dir, regularizer,
        ...
        dir_id = 'reg_{}_L_{}_N_{}_WN_{}'.format(
    ...
    def encode_dir_name(cls, model):
        model_dir_name = 'reg_{}_L_{}_N_{}_WN_{}'.format(
...
class ResidualTN(nn.Module):
    ...
    def load_best_model(cls, models_dir, regularizer,
                        freq_res, sample_res, weighted_norm):
        dir_id = 'residualmask_new_{}_L_{}_N_{}_WN_{}'.format(
            regularizer, sample_res, freq_res, weighted_norm)
        dir_path = os.path.join(models_dir, dir_id)
        best_path = glob2.glob(dir_path + '/best_*')[0]
        return cls.load(best_path)
    ...
    @classmethod
    def encode_dir_name(cls, model):
        model_dir_name = 'residualmask_new_{}_L_{}_N_{}_WN_{}'.format(
            model.afe_reg, model.L, model.N, model.weighted_norm)
        return model_dir_name
