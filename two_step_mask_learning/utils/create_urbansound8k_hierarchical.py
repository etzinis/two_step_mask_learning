"""!
@brief Data Preprocessor for urbansound8k in order to convert the dataset to an
...
def write_data_wrapper_func(audio_files_dir,
    ...
    def process_class(sound_class):
        class_path = os.path.join(audio_files_dir, sound_class)
        audio_paths = glob(class_path + '/*.wav')

        for path in audio_paths:
            uid = os.path.basename(path).split('.wav')[0]
            ...
            output_uid_folder = os.path.join(
                output_dirpath, sound_class, uid)
    ...
    return lambda uid: process_class(uid)


def convert_ESC50_to_hierarchical_dataset(input_dirpath,
                                          output_dirpath,
                                          wav_timelength):
    """! This function converts the UrbanSound8K dataset in a universal
    dataset with all different sound classes in separate folders.

    Args:
        input_dirpath: The path of UrbanSound8K dataset
    ...
    sound_classes_folders = glob(input_dirpath + '/s_*')
    sound_classes = set([os.path.basename(f) for f in sound_classes_folders])
    ...
    progress_display.progress_bar_wrapper(
        write_data_func,
        list(sound_classes),
        message='Processing {} classes...'.format(len(sound_classes)))

    print("Dataset ready at: {}".format(output_dirpath))


def example_of_usage():
    input_dirpath = '/mnt/data/UrbanSound8K/data/train'
    output_dirpath = '/mnt/data/hierarchical_sound_datasets/UrbanSound8K'
    wav_timelength = 4
